document.addEventListener('DOMContentLoaded', () => {
    ...
    fetchGuides('/api/guides?category=Linux&limit=3', displayPopularGuides);
    ...
    function initializeSearch() {
        ...
        searchInput.addEventListener('input', (e) => {
            ...
            debounceTimeout = setTimeout(() => {
                fetchGuides(`/api/guides/search?q=${encodeURIComponent(query)}&category=Linux`, (guides) => {
        ...
        searchInput.addEventListener('keydown', (e) => {
            if (e.key === 'Enter') {
                ...
                const query = e.target.value.trim();
                if (query.length > 0) {
                    fetchGuides(`/api/guides/search?q=${encodeURIComponent(query)}&category=Linux`, displaySearchResults);
                    searchSuggestions.classList.add('hidden');
                }
            }
        });
