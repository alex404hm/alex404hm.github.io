document.addEventListener('DOMContentLoaded', () => {
    ...
    fetchGuides('/api/guides?category=iOS&limit=3', displayPopularGuides);
    ...
    function initializeSearch() {
        ...
        searchInput.addEventListener('input', (e) => {
            ...
            debounceTimeout = setTimeout(() => {
                fetchGuides(`/api/guides/search?q=${encodeURIComponent(query)}&category=iOS`, (guides) => {
        ...
        searchInput.addEventListener('keydown', (e) => {
            if (e.key === 'Enter') {
                ...
                const query = e.target.value.trim();
                if (query.length > 0) {
                    fetchGuides(`/api/guides/search?q=${encodeURIComponent(query)}&category=iOS`, displaySearchResults);
                    searchSuggestions.classList.add('hidden');
                }
            }
        });
